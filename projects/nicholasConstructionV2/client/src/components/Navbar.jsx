...
import {navLinks} from '../textConstants'
...
export const Navbar = () => {
  ...
  return (
    <nav className='w-full flex py-3 stikcy top-0 z-10
    justify-between items-center navbar sticky'>
      <img src={logo} alt=""
      className='w-[200px]' />
      <ul className='list-none sm:flex hidden
      justify-end items-center flex-1 me-6'>
        {navLinks.map((nav, index) => (
          ...
          'mr-0' : 'mr-10'} mr-10`}>
            <a className='d-none p-3' href={nav.link}>{nav.title}</a>
          </li>
        ))}
          <li>
            <BookingPopUp />
      ...
      justify-end items-center'>
        ...
        mx-4 my-2 min-w[140px] rounded-xl sidebar`}>
            <ul className='list-none flex-inline
              ...
              ${index === navLinks.length - 1 ? 
              'mr-0' : 'mb-4'} mr-10`}>
              <a className='font-bold ' href={nav.link}>{nav.title}</a>
            </li>
          ))}
          <li>
            <BookingPopUp />
